...
export const generateScreen = (screensPath: string, screen: Screen) => {
  ...
  const screenFile = executeTemplate('component.js', { name: fileName, children: childrenToRender, components: componentNames })
  write(path.join(screensPath, `${fileName}.js`), screenFile)
}
